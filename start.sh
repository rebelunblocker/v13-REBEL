...
print_bold_with_outline "Welcome to Rebel V15, user!"
...
echo -e "${BLUE}[3/4] Starting Rebel Unblocker...${RESET}"
npm start

if [ $? -eq 0 ]; then
    echo -e "${GREEN}[Success] Rebel Unblocker started successfully.${RESET}"
    echo -e "${YELLOW}[Info] You can access Rebel V15 via localhost.${RESET}"
else
    echo -e "${RED}[Error] Failed to start Rebel Unblocker. Please check the application logs for details.${RESET}"
    exit 1
fi
